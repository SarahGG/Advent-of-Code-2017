package DayFourHighEntropyPassphrases;
...
public class DayFour {
    public static void main(String[] args) {
        String passphraseString = "vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh\n" +
        ...
        List<String> unsplitPassphrasseList = Arrays.asList(passphraseString.split("[\n]"));
        ArrayList<List<String>> passphraseList = new ArrayList<>();
        for(String unsplitPassphrase : unsplitPassphrasseList) {
            passphraseList.add(Arrays.asList(unsplitPassphrase.split("[\\s]")));
        }

        int validPassphrases = 0;

        //looking at each passphrase
        for(List<String> passPhrase : passphraseList) {
            boolean passphraseIsValid = true;
            int copyCounter;

            //uses each individual word as the mold and tries to find a match
            for(String password : passPhrase) {
                copyCounter = 0;
                //looks at every word in the phrase. If a match is found, a counter is incremented
                //if a passphrase is valid, the counter sohuld only ever equal one
                for(String testingPassword : passPhrase) {
                    if(testingPassword.matches(password)) {
                        copyCounter += 1;
                    }
                }

                //if password is matches with more than itself, then this whole passphrase is bunk
                if (copyCounter != 1) {
        ...
        System.out.println("valid phrases: " + validPassphrases);
    }
}
