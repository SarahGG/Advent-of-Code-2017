 ...
 * @version 1.0
 */
public class DaySevenRecursiveCircus {
    public static void main(String[] args) {
        String nameList = "jlbcwrl (93)\n" +
        ...
        ArrayList<DiskTower> diskTowerArrayList = new ArrayList<>();
        List<String> diskTowerList = Arrays.asList(nameList.split("\n"));

        for(String diskIndetity : diskTowerList) {
            diskTowerArrayList.add(new DiskTower(diskIndetity));
        }

        for(DiskTower diskTower : diskTowerArrayList) {
            diskTower.initParent(diskTowerArrayList);
        }

        for(DiskTower diskTower: diskTowerArrayList) {
            if (diskTower.getParent() == null) {
                System.out.println("the parent disk is: " + diskTower.getName());
            }
        }
    }
}
